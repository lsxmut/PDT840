...
@FXMLController
@Slf4j
public class NdaDataTEVAnalysisController extends NdaBaseController implements Initializable {
    @FXML
    CheckBox qzCheckBox;
    @FXML
    CheckBox qxCheckBox;
...
//                    setStyle("-fx-background-color: red");
                    ...
                    setTooltip(new Tooltip() {{
                        ...
                        setText("" + dataDeviceDto.getDeviceName() + "；\n" +
                                "地电波总分析结果（前、后综合获得）—" + getNdaResult(tlab, 0, tevXjDto == null ? -1 : tevXjDto.getX21(), tevAttentionDegree) + "；\n" +
                                "超声波总分析结果—" + getNdaResult(tlab, 1, aaXjDto == null ? -1 : aaXjDto.getX21(), aaAttentionDegree) + "；\n" +
                                "地电波和超声波综合分析结果—" + getNdaResult(tlab, 2, -1, (tevAttentionDegree > aaAttentionDegree ? tevAttentionDegree : aaAttentionDegree)) + "；");
                    }});
                }});
            }
...
//            fillChart(result);
    ...
    private String getNdaResult(Label tlab, int type, int source, int degree) {
        String result = "";
