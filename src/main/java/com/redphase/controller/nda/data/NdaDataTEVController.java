...
import com.redphase.view.nda.data.NdaDataTEVDetailView;
import com.redphase.view.nda.data.NdaDataTEVSiteInfoView;
...
@FXMLController
@Slf4j
public class NdaDataTEVController extends NdaBaseController implements Initializable {
    @FXML
...
//    Button reportButton;//
    ...
    @Autowired
    private NdaDataTEVSiteInfoView ndaDataTEVSiteInfoView;
    @Autowired
    private NdaDataTEVSiteInfoController ndaDataTEVSiteInfoController;
    @Autowired
    private NdaDataTEVAnalysisController ndaHorizontalAnalysisController;
    @Autowired
    private NdaDataTEVDetailView ndaDataTEVDetailView;
    @Autowired
    private NdaDataTEVDetailController ndaDataTEVDetailController;
...
//                            Double height = Application.getStage().getHeight() - 100;
                            idialog.openDialog(tableDto.getDataDeviceDto().getDeviceName(), ndaDataTEVSiteInfoView, 1180.0, 700.0, true);
                            ehcacheUtil.setCache(CommonConstant.NDA_DATA_SKILL_DEVICE_SITE, tableDto.getDataDeviceDto());
                            ndaDataTEVSiteInfoController.loadData();
                        });
                    }
                }
            });
...
//        Double width = Application.getStage().getWidth() - 200;
//        Double height = Application.getStage().getHeight() - 100;
        idialog.openDialog("横向分析表", ndaHorizontalAnalysisView, 1150.0, 745.0, false);
        ndaHorizontalAnalysisController.loadChartData(fullNameL.getText());
    }
...
//        Double width = Application.getStage().getWidth() - 200;
//        Double height = Application.getStage().getHeight() - 100;
        idialog.openDialog("开关柜详情", ndaDataTEVDetailView, 1300.0, 745.0, false);
        ndaDataTEVDetailController.init(dataDeviceList);
    }
