...
import com.redphase.view.atlas.data.AtlasDataPhaseView;
import com.redphase.view.atlas.data.AtlasDataSiteCountView;
...
@FXMLController
@Slf4j
public class AtlasSiteCountController extends AtlasBaseController implements Initializable {
    @Autowired
    AtlasDataPhaseView atlasDataPhaseView;
    @Autowired
    AtlasDataSiteCountView atlasDataSiteCountView;
    @Autowired
    AtlasDataPhaseController atlasDataPhaseController;
    @Autowired
    DiagAe diagAe;
    @Autowired
    DiagUhf diagUhf;
    ...
    public void init(String testTechnology, List<DataTableDto> dataTableDtos, Map map) {
        ...
        Parent nodeView = atlasDataSiteCountView.getView();
...
//            }
            idialog.setTitle(atlasDataSiteCountView, dataDeviceDto.getSubstation() + dataDeviceDto.getDeviceName() + dataDeviceSiteDto.getSiteName());
...
//        Double height = Application.getStage().getHeight() - 100;
        idialog.openDialog(dataDeviceDto.getSubstation() + dataDeviceDto.getDeviceName() + dataDeviceSiteDto.getSiteName(), atlasDataPhaseView, 1300.0, 768.0,false);
        atlasDataPhaseController.init(testTechnology, atlasArr, new HashMap() {{
