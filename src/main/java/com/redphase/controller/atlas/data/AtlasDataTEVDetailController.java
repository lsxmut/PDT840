...
@FXMLController
@Slf4j
public class AtlasDataTEVDetailController extends AtlasBaseController implements Initializable {
    @FXML
...
//            //查询所有地电波巡检检测数据
    ...
    private Pane addAtlas(DataDeviceDto deviceDto) {
        Pane atlasPane = new Pane() {{
            setPrefWidth(620);
            prefHeight(450);
            getStyleClass().setAll("tt");
        }};
        FlowPane.setMargin(atlasPane, new Insets(5, 5, 5, 5));
        atlasPane.getChildren().add(new Label("地电波(dBmV)") {{
        ...
        atlasPane.getChildren().add(new Label("" + deviceDto.getDeviceName()) {{
            setAlignment(Pos.CENTER);
            setPrefHeight(65);
            setPrefWidth(620);
        ...
        atlasPane.getChildren().add(new Canvas() {{
            setHeight(300);
            setWidth(550);
            setLayoutX(50);
            setLayoutY(65);
...
//            gc.fillText(""+deviceName,prpdX+(i * xStep)+xStep/4,prpdY+15);
        ...
        atlasPane.getChildren().add(getScaleplateHBox());
        return atlasPane;
    }
