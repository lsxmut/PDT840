...
/** <p> 江苏_超声_波形图数据 DTO */
public class JSDataAeBXDto extends BaseDto {
    /**
    ...
    private Integer x30;
    /**
     * 必备 int32 4字节 数据点数n - 数据点个数。
     */
    private Integer x31;
    /**
     * 必备 int64 8字节 采样率 - 单位SPS。
     */
    private Long x32;
    /**
    ...
    private String x33;
    /**
     * 可选 自定义 145字节 预留 - 预留为厂家自定义可选字段。
     */
    private String x34;
    /**
     * 必备 d[n] k*n字节 超声波形图数据 - 根据存储数据类型t获取数据的存储方式。实例1：t是0x02，d表示uint8数组，k=1；实例2：t是0x04，d表示int32数组，k=4；实例3：t是0x06，d表示float数组，k=4。依次存储每个数据点的信号幅值Q，存储顺序：Q[0]；Q[1]；……；Q[n-1]。
     */
    private String x35;
    /**
     * 必备 - 32字节 预留 - 预留
