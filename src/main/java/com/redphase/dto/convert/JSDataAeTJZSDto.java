...
/** <p> 江苏_超声_相位图数据(噪声) DTO */
public class JSDataAeTJZSDto extends BaseDto {
    /**
    ...
    private String x32;
    /**
     * 可选 自定义 153字节 预留 - 预留为厂家自定义可选字段。
     */
    private String x33;
    /**
     * 必备 d[2*n] k*2*n字节 超声相位图数据 - 根据存储数据类型t获取数据的存储方式。实例1：t是0x02，d表示uint8数组，k=1；实例2：t是0x04，d表示int32数组，k=4；实例3：t是0x06，d表示float数组，k=4。依次存储每个数据点的相位Phase和幅值Q，存储顺序： Phase[0]，Q[0]；Phase[1]，Q[1]；……；Phase[n-1]，Q[n-1]。相位值范围[0~360)，单位为°。
     */
    private String x34;
    /**
     * 必备 - 32字节 预留 - 预留
