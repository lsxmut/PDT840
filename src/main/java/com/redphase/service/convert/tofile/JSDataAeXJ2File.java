...
import com.redphase.api.atlas.ae.IDataAeXjService;
...
/** <p> 江苏_超声_特征图数据 */
@Slf4j
public class JSDataAeXJ2File extends BaseData2File {

    public static void toFile(Map paramMap) throws Exception {
        FileOutputStream out = null;
        try {
            //AE巡检业务类
            IDataAeXjService dataAeXjService = (IDataAeXjService) paramMap.get("dataAeXjService");
            //资源记录
            ...
            String dataFilePath = getPathAndMkdirs(paramMap.get("outFilePath") + File.separator + "AE") + File.separator + "AE_XJ_" + dataAeXjDto.getX3() + ".dat";
            //数据存储
            out = new FileOutputStream(dataFilePath);
            //图谱数据
            byte[] atlasArr = {};// JSONObject.parseObject(dataAeXjDto.getX11(), byte[].class);
            //文件长度计算
            int dataRowLength = 0;
            int[] dataRowBytes = JSDataType.AE_XJ.getDataRowBytes();
...
//     3    * 必备 int64 8字节 文件生成时间 - 生成文件的时间，格式为YYYYMMDDhhmmssfff，例如20100818151010001。
            byte[] x3buffer = ByteConverterUtil.long2bytes(dataAeXjDto.getX3());
            if (x3buffer != null) {
...
//     9    * 必备 char 32字节 仪器厂家 - 使用UNICODE编码。以0x0000结尾，例如：HuaCheng。
            byte[] x9buffer = ByteConverterUtil.unicode2byte(dataAeXjDto.getX24());
            if (x9buffer != null) {
                System.arraycopy(x9buffer, 0, bytes, destPost, x9buffer.length);
            }
            destPost += 32;
//     10    * 必备 char 32字节 仪器型号 - 使用UNICODE编码。以0x0000结尾，例如：PDS-T95。
            byte[] x10buffer = ByteConverterUtil.unicode2byte(dataAeXjDto.getX25());
            if (x10buffer != null) {
...
//     11    * 可选 uint8 4字节 仪器版本号 - 所使用的数据格式规范版本号。版本号有4个部分，形如X.X.X.X。实例：版本号为1.0.0.0，数组元素[0]到[3]分别存储1、0、0、0四个数字。每个元素可以是整数0至255中的某数。
            System.arraycopy(JSONObject.parseObject(dataAeXjDto.getX26(), byte[].class), 0, bytes, destPost, 4);
            destPost += 4;
//     12    * 必备 char 32字节 仪器序列号 - 即仪器出厂编号，使用ASCII编码。以\0结尾，例如：S12300000000000000。
            byte[] x12buffer = ByteConverterUtil.string2byte(dataAeXjDto.getX27());
            if (x12buffer != null) {
                System.arraycopy(x12buffer, 0, bytes, destPost, x12buffer.length);
            }
            destPost += 32;
//     13    * 必备 float 4字节 系统频率 - 被测设备电压频率，单位Hz。例如50Hz。
            System.arraycopy(ByteConverterUtil.float2byte(dataAeXjDto.getX18()), 0, bytes, destPost, 4);
            destPost += 4;
...
//     15    * 必备 float 4字节 同步频率 - 采集装置的同步频率，单位Hz。
            System.arraycopy(ByteConverterUtil.float2byte(dataAeXjDto.getX19()), 0, bytes, destPost, 4);
            destPost += 4;
//     16    * 可选 自定义 220字节 预留 - 预留为厂家自定义可选字段。
            destPost += 220;
//     17    * 必备 uint8 1字节 图谱类型编码 - 标识该文件的图谱类型。参见图谱类型编码表。
            bytes[destPost] = JSAtlasType.AE_XJ_超声特征图.value;
            destPost += 1;
...
//     19    * 必备 int64 8字节 图谱生成时间 - 生成图谱的时间，格式为YYYYMMDDhhmmssfff，例如20100818151010001。
            System.arraycopy(ByteConverterUtil.long2bytes(dataAeXjDto.getX3()), 0, bytes, destPost, 8);
            destPost += 8;
...
//     27    * 必备 uint8 1字节 幅值单位 - 表示幅值的单位。参见幅值单位表。
            bytes[destPost] = fzdw2byte(dataAeXjDto.getX11(),JSAtlasType.UNTIL_AE_XJ.value);//new Byte(dataAeXjDto.getX11()).byteValue();
            destPost += 1;
//     28    * 必备 float 4字节 幅值下限 - 仪器所能检测到的信号幅值的下限。
            System.arraycopy(ByteConverterUtil.float2byte(dataAeXjDto.getX12()), 0, bytes, destPost, 4);
            destPost += 4;
//     29    * 必备 float 4字节 幅值上限 - 仪器所能检测到的信号幅值的上限。
            System.arraycopy(ByteConverterUtil.float2byte(dataAeXjDto.getX13()), 0, bytes, destPost, 4);
            destPost += 4;
...
//     33    * 必备 uint8 8字节 放电类型概率 - 表示仪器诊断结果的放电类型概率。参见放电类型概率定义。
            byte[] x36buffer = JSONObject.parseObject(dataAeXjDto.getX10(), byte[].class);
            if (x36buffer != null) {
                System.arraycopy(x36buffer, 0, bytes, destPost, x36buffer.length);
            }
            destPost += 8;
//     34    * 必备 float 4字节 信号最大值 - 信号的最大值。
            System.arraycopy(ByteConverterUtil.float2byte(dataAeXjDto.getX5()), 0, bytes, destPost, 4);
            destPost += 4;
//     35    * 必备 float 4字节 信号有效值 - 信号的均方根值。
            System.arraycopy(ByteConverterUtil.float2byte(dataAeXjDto.getX7()), 0, bytes, destPost, 4);
            destPost += 4;
//     36    * 必备 float 4字节 频率分量1相关性 - 超声信号中被测系统频率分量的幅值。
            System.arraycopy(ByteConverterUtil.float2byte(dataAeXjDto.getX8()), 0, bytes, destPost, 4);
            destPost += 4;
//     37    * 必备 float 4字节 频率分量2相关性 - 超声信号中两倍被测系统频率分量的幅值。
            System.arraycopy(ByteConverterUtil.float2byte(dataAeXjDto.getX9()), 0, bytes, destPost, 4);
            destPost += 4;
