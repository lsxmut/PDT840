...
import com.redphase.api.atlas.hfct.IDataHfctTjzsService;
...
@Data
/** <p> 江苏_高频电流_PRPD图和PRPS图数据(噪声) */
@Slf4j
public class JSDataHfctTJZS2File extends BaseData2File {

    public static void toFile(Map paramMap) throws Exception {
        FileOutputStream out = null;
        try {
            //HFCT统计噪声业务类
            IDataHfctTjzsService dataHfctTjzsService = (IDataHfctTjzsService) paramMap.get("dataHfctTjzsService");
            //资源记录
            ...
            String dataFilePath = getPathAndMkdirs(paramMap.get("outFilePath") + File.separator + "HFCT") + File.separator + "HFCT_TJZS_" + dataHfctTjzsDto.getX14() + ".dat";
            //数据存储
            out = new FileOutputStream(dataFilePath);
            //图谱数据
            float[][] atlasArr = JSONObject.parseObject(dataHfctTjzsDto.getX11(), float[][].class);
            //文件长度计算
            int dataRowLength = 0;
            int[] dataRowBytes = JSDataType.HFCT_TJZS.getDataRowBytes();
...
//     3	* 必备 int64 8字节 文件生成时间 - 生成文件的时间，格式为YYYYMMDDhhmmssfff，例如20100818151010001。
            byte[] x3buffer = ByteConverterUtil.long2bytes(dataHfctTjzsDto.getX14());
            if (x3buffer != null) {
...
//     9	* 必备 char 32字节 仪器厂家 - 使用UNICODE编码。以0x0000结尾，例如：HuaCheng。
            byte[] x9buffer = ByteConverterUtil.unicode2byte(dataHfctTjzsDto.getX31());
            if (x9buffer != null) {
                System.arraycopy(x9buffer, 0, bytes, destPost, x9buffer.length);
            }
            destPost += 32;
//     10	* 必备 char 32字节 仪器型号 - 使用UNICODE编码。以0x0000结尾，例如：PDS-T95。
            byte[] x10buffer = ByteConverterUtil.unicode2byte(dataHfctTjzsDto.getX32());
            if (x10buffer != null) {
...
//     11	* 可选 uint8 4字节 仪器版本号 - 所使用的数据格式规范版本号。版本号有4个部分，形如X.X.X.X。实例：版本号为1.0.0.0，数组元素[0]到[3]分别存储1、0、0、0四个数字。每个元素可以是整数0至255中的某数。
            System.arraycopy(JSONObject.parseObject(dataHfctTjzsDto.getX33(), byte[].class), 0, bytes, destPost, 4);
            destPost += 4;
//     12	* 必备 char 32字节 仪器序列号 - 即仪器出厂编号，使用ASCII编码。以\0结尾，例如：S12300000000000000。
            byte[] x12buffer = ByteConverterUtil.string2byte(dataHfctTjzsDto.getX34());
            if (x12buffer != null) {
                System.arraycopy(x12buffer, 0, bytes, destPost, x12buffer.length);
            }
            destPost += 32;
//     13	* 必备 float 4字节 系统频率 - 被测设备电压频率，单位Hz。例如50Hz。
            System.arraycopy(ByteConverterUtil.float2byte(dataHfctTjzsDto.getX22()), 0, bytes, destPost, 4);
            destPost += 4;
...
//     15	* 必备 float 4字节 同步频率 - 采集装置的同步频率，单位Hz。
            System.arraycopy(ByteConverterUtil.float2byte(dataHfctTjzsDto.getX21()), 0, bytes, destPost, 4);
            destPost += 4;
//     16	* 可选 自定义 220字节 预留 - 预留为厂家自定义可选字段。
            destPost += 220;
//     17	* 必备 uint8 1字节 图谱类型编码 - 标识该文件的图谱类型。参见图谱类型编码表。
            bytes[destPost] = JSAtlasType.HFCT_TJZS_高频PRPS图.value;
            destPost += 1;
...
//     19	* 必备 int64 8字节 图谱生成时间 - 生成图谱的时间，格式为YYYYMMDDhhmmssfff，例如20100818151010001。
            System.arraycopy(ByteConverterUtil.long2bytes(dataHfctTjzsDto.getX14()), 0, bytes, destPost, 8);
            destPost += 8;
...
//     27	* 必备 uint8 1字节 幅值单位 - 表示幅值的单位。参见幅值单位表。
            bytes[destPost] = fzdw2byte(dataHfctTjzsDto.getX6(),JSAtlasType.UNTIL_HFCT_TJ.value);//new Byte(dataHfctTjzsDto.getX6()).byteValue();
            destPost += 1;
//     28	* 必备 float 4字节 幅值下限 - 仪器所能检测到的信号幅值的下限。
            System.arraycopy(ByteConverterUtil.float2byte(dataHfctTjzsDto.getX7()), 0, bytes, destPost, 4);
            destPost += 4;
//     29	* 必备 float 4字节 幅值上限 - 仪器所能检测到的信号幅值的上限。
            System.arraycopy(ByteConverterUtil.float2byte(dataHfctTjzsDto.getX8()), 0, bytes, destPost, 4);
            destPost += 4;
//     30	* 必备 uint8 1字节 频带 - 表示滤波类型0xFF:未记录0x01:全通0x02:低通0x03:高通0x04:带通0x05:带阻0x06:可扩充
            bytes[destPost] = 0x04;
            destPost += 1;
//     31	* 必备 float 4字节 下限频率 - 表示检测仪器的下限频率，单位Hz。
            System.arraycopy(ByteConverterUtil.float2byte(dataHfctTjzsDto.getX29()), 0, bytes, destPost, 4);
            destPost += 4;
//     32	* 必备 float 4字节 上限频率 - 表示检测仪器的上限频率，单位Hz。
            System.arraycopy(ByteConverterUtil.float2byte(dataHfctTjzsDto.getX30()), 0, bytes, destPost, 4);
            destPost += 4;
//     33	* 必备 int32 4字节 相位窗数m - 工频周期被等分成m个相位窗，每个相位窗跨360/m度。
            System.arraycopy(ByteConverterUtil.int2byte(dataHfctTjzsDto.getX4()), 0, bytes, destPost, 4);
            destPost += 4;
//     34	* 必备 int32 4字节 量化幅值n - 幅值范围的等分区间数。如果该文件是PRPS图谱，则该4个字节清零，赋值为：0x00000000。
            System.arraycopy(ByteConverterUtil.int2byte(dataHfctTjzsDto.getX5()), 0, bytes, destPost, 4);
            destPost += 4;
//     35	* 必备 int32 4字节 工频周期数p - 图谱工频周期的个数。如果该文件是PRPD图谱，则该4个字节清零，赋值为：0x00000000。
            System.arraycopy(ByteConverterUtil.int2byte(dataHfctTjzsDto.getX9()), 0, bytes, destPost, 4);
            destPost += 4;
//     36	* 必备 uint8 8字节 放电类型概率 - 表示仪器诊断结果的放电类型概率。参见放电类型概率定义。
            byte[] x36buffer = JSONObject.parseObject(dataHfctTjzsDto.getX10(), byte[].class);
            if (x36buffer != null) {
