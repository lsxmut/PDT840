...
public class AnalyzeAEData {
     ...
     * |  11  |  uint8     |  1字节   |  幅值单位  表示幅值的单位 mV: 0x06
     * |  12  |  float     |  4字节   |  幅值下限  仪器所能检测到的放电信号幅值的下限。暂定：0
     * |  13  |  float     |  4字节   |  幅值上限  仪器所能检测到的放电信号幅值的上限。暂定：300
     * |  14  |  float     |  4字节   |  参考阈值
     * |  15  |  char      |  1字节   |  增益  20dB: 0x01 40dB: 0x02 60dB: 0x03
     * |  16  |  float     |  4字节   |  f1分量  检测参数，用户人工录入或点击获取的值。
     * |  17  |  float     |  4字节   |  f2分量  检测参数，f2=f1×2；
     * |  18  |  float     |  4字节   |  系统频率
     * |  19  |  float     |  4字节   |  同步频率  同步频率=f1分量
     * |  20  |  float     |  4字节   |  警戒值1  如5
     * |  21  |  float     |  4字节   |  警戒值2  如15
     * |  22  |  float     |  4字节   |  警戒值3  如20
     * |  23  |  int       |  4字节   |  前置增益  单位dB
     ...
     * | 11   |   char     |   1字节   |   幅值单位 表示幅值的单位 mV: 0x06
     * | 12   |   float    |   4字节   |   幅值下限 仪器所能检测到的放电信号幅值的下限。暂定：0
     * | 13   |   float    |   4字节   |   幅值上限 仪器所能检测到的放电信号幅值的上限。暂定：300
     * | 14   |   float    |   4字节   |   参考阈值
     * | 15   |   char     |   1字节   |   增益 20dB: 0x01 40dB: 0x02 60dB: 0x03
     ...
     * | 21   |   float    |   4字节    |   同步频率
     * | 22   |   float    |   4字节    |   系统频率
     * | 23   |   char     |   1字节    |   放大增益 20dB: 0x01 40dB: 0x02 60dB: 0x03
     * | 24   |   float    |   4字节    |   警戒值1 如5
     * | 25   |   float    |   4字节    |   警戒值2 如15
     * | 26   |   float    |   4字节    |   警戒值3 如20
     * | 27   |   int      |   4字节    |   前置增益 单位dB
     * | 28   |   char     |   32字节   |   仪器厂家 从系统设置中提取，使用UNICODE编码。以0x0000结尾，例如：RedPhase。
     * | 29   |   char     |   32字节   |   仪器型号 从系统设置中提取，使用UNICODE编码。以0x0000结尾，例如：PDT-400。
     * | 30   |   uint8    |   4字节    |   仪器版本号 从系统设置中提取，所使用的数据格式规范版本号。版本号有4个部分，形如X.X.X.X。 实例：版本号为1.0.0.0，数组元素[0]到[3]分别存储1、0、0、0四个数字。每个元素可以是整数0至255中的某数。
     * | 31   |   char     |   32字节   |   仪器序列号 从系统设置中提取，即仪器出厂编号，使用ASCII编码。以\0结尾，例如：S12300000000000000。
     * | 32   |   int      |   4字节    |   检验和 13到校验和前一项的数据校验。
     * ---------------------------------------------------------------------------------------
     */
     ...
     * |  7   |   char       |   1字节      |   幅值单位 表示幅值的单位 mV: 0x06
     ...
     * | 15   |   float      |   4字节      |   同步频率
     * | 16   |   float      |   4字节      |   系统频率
     * | 17   |   char       |   1字节      |   放大增益 20dB: 0x01 40dB: 0x02 60dB: 0x03
     * | 18   |   float      |   4字节      |   警戒值1 如5
     * | 19   |   float      |   4字节      |   警戒值2 如15
     * | 20   |   float      |   4字节      |   警戒值3 如20
     * | 21   |   int        |   4字节      |   前置增益 单位dB
